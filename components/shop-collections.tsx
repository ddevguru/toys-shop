        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {collections.map((collection, index) => (
            <Link
              ...
              className="group relative rounded-3xl overflow-hidden aspect-square transition-all duration-500 hover:shadow-2xl hover:shadow-black/30 hover:translate-y-[-12px] border-3 border-muted hover:border-primary/50 animate-slideUp"
            ...
            >
              ...
              {/* Content */}
              <div className="absolute inset-0 flex flex-col items-center justify-end p-6 text-center space-y-4">
                <h3
                  className="text-2xl md:text-3xl font-bold text-white leading-tight text-balance"
                  style={{ fontFamily: 'var(--font-heading)' }}
                >
                  {collection.name}
                </h3>
                <p className="text-sm text-white/90 font-medium hidden md:block">
                  {collection.description}
                </p>
                <div className="inline-block bg-white text-foreground hover:bg-white/90 px-6 py-2 rounded-full text-sm font-bold transition-all duration-300 shadow-lg transform group-hover:scale-110 group-hover:shadow-xl">
                  Shop Now →
                </div>
              </div>
            </Link>
